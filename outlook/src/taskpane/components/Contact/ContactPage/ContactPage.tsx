...
class ContactPage extends React.Component<ContactPageProps, ContactPageState> {
        ...
        partnerRequest.promise
            .then((response) => {
                ...
                if (parsed.result.leads) {
                    partner.leads = parsed.result.leads.map((lead_json) =>
                        Lead.fromJSON(lead_json),
                    );
                }
                if (parsed.result.tasks) {
                    partner.tasks = parsed.result.tasks.map((task_json) =>
                        Task.fromJSON(task_json),
                    );
                }
                if (parsed.result.tickets) {
                    partner.tickets = parsed.result.tickets.map((ticket_json) =>
    ...
    render() {
        if (this.state.isLoading) {
            ...
            let hideCollapseButton =
                !this.isHelpdeskInstalled() && !this.isCrmInstalled();
