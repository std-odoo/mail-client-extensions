...
class Main extends React.Component<{}, MainState> {
        ...
        CancellableMatchedPartnersRequest.promise
            .then((response) => {
                ...
                if (partners.length > 0) {
                    ...
                    const newPartner = PartnerData.createNewPartnerFromEmail(
                        displayName,
                        email,
                    );
                ...
                cancellableRequest.promise
                    .then((response) => {
                        ...
                        if ('error' in parsed.result) {
                            ...
                            partner.company.enrichmentStatus =
                                EnrichmentStatus.enrichmentEmpty;
                            this.setState({
                                matchedPartners: [partner],
                                selectedPartner: partner,
                                partnersLoading: false,
    ...
    private onSearchClick = (state) => {
        if (this.context.isConnected()) {
            ...
            if (
                this.state.backStack.length > 0 ||
                this.state.matchedPartners.length == 1
            ) {
    ...
    render() {
        ...
        let connectionButton = (
            <div
                ...
                onClick={() => this.context.navigation.goToLogin()}>
                Login
            </div>
        );
        if (this.context.isConnected()) {
            ...
            mainContent = (
                <div>
                    <Search
                        ...
                        historyState={
                            this.state.poppedElement &&
                            this.state.poppedElement.type == BackStackItemType.query
