...
class CompanySection extends React.Component<CompanySectionProps, CompanySectionState> {
        ...
        enrichRequest.promise
            .then((response) => {
                ...
                if (enrichResponse.result['enrichment_info']) {
                    ...
                    enrichResponse.result.error &&
                    enrichResponse.result.error.length
                ) {
                    this.context.showTopBarMessage(
                        new EnrichmentInfo(
                            EnrichmentInfoType.OdooCustomError,
                            enrichResponse.result.error,
                        ),
                    );
    ...
    private enrichAndUpdate = () => {
        ...
        enrichRequest.promise
            .then((response) => {
                ...
                const company = Company.fromJSON(
                    enrichResponse.result.company,
                    enrichmentInfo,
                );
    ...
    private getCompanyInsightsSection = () => {
        ...
        if (companyYearFounded) {
            companyInsights.push(
                <CompanyInfoItem
                    icon={faInfo}
                    title={_t('Year founded')}
                    value={'' + company.getYearFounded()}
                />,
            );
        }
        ...
        if (!companyInsights.length && parentIsCompany) {
            if (company.enrichmentStatus != EnrichmentStatus.enrichmentEmpty) {
                return (
                    <div className="insights-empty-info muted-text">
                        {_t('No extra information found')}
                    </div>
                );
            } else {
                return null;
        ...
        if (this.state.company.isAddedToDatabase()) {
            return (
                <div>
                    ...
                    <div>
                        <div
                            className="odoo-secondary-button insights-button"
    ...
    private getCompanyEnrichmentEmptySection = () => {
        ...
        return (
            <div>
                {this.state.company.isAddedToDatabase() &&
                    this.getCompanyInsightsSection()}
    ...
    private openInOdoo = () => {
        ...
        const url =
            api.baseURL +
            `/web#id=${this.props.partner.company.id}&model=res.partner&view_type=form`;
